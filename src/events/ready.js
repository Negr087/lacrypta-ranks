...
const event = {
    ...
    execute: async (_client) => {
        ...
        await (0, deployCommands_1.deployCommands)(_client);
        ...
        for (const [guildId, guild] of _client.guilds.cache) {
            try {
                ...
                // --- GUILD ---
                await cache_1.cacheService.upsertGuild(guildId);
                console.log(`✅ Guild upserted en DB: ${guild.name}`);
                // --- ROLES ---
                console.log(`🎭 Roles encontrados: ${guild.roles.cache.size}`);
                for (const role of guild.roles.cache.values()) {
                    try {
                        await cache_1.cacheService.upsertRole(guildId, role.id, role.name);
                ...
                for (const member of members.values()) {
                    if (!member.user.bot) {
                        try {
                            await cache_1.cacheService.upsertMember(guildId, member.id, member.displayName, member.displayAvatarURL());
        ...
        setInterval(async () => {
            try {
                const updateLevelStatus = await cache_1.cacheService.updateMembersLevelsToDatabase();
